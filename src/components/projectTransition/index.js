...
const ProjectTransition = ({ nextProject }) => {
  ...
  const setPatch = useCallback(() => {
    const { projectLogoSvg, projectTitleRich } = nextProject
    ...
    projectPatchImg.innerHTML = projectLogoSvg
    projectPatchTitle.innerHTML = ReactDOMServer.renderToStaticMarkup(
      projectTitleRich.map((t, i) => <span key={i}>{t.text}</span>)
    )
  }, [nextProject])
  ...
  const img = () => ({ __html: nextProject.projectLogoSvg })
